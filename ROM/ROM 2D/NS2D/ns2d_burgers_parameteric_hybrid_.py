...
plot_data(x,y,PHIw[:,:,-1])#,aGP[-1,:,:],aGP1[-1,:,:]) 
...
for n in range(ns+1):
   ...
   upTest[:,n] = 0.1*uTest[:,n] #perturbation (unknown physics)
...
def plot_data(t,aTrue,aGPtest,aGPml,aGPmlc,filename):
    fig, ax = plt.subplots(nrows=4,ncols=2,figsize=(12,8))
    ax = ax.flat
    nrs = aTrue.shape[1]
...
plot_data(t,aTest,aGPtest,aGPml,aGPmlc,'hybrid_ns2d.pdf') 
...
plot_data(t,at[:,:],aTest,'modes_2d.pdf')#,aGP1[-1,:,:])
