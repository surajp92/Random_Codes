...
def GrassInt2(Phi,pref,p,pTest):
    # Phi is input basis [training]
    # pref is the reference basis [arbitrarty] for Grassmann interpolation
    # p is the set of training parameters
    # pTest is the testing parameter
...
for p in range(0,nc):
    for n in range(0,ns+1):
        ...
        for i in range(nx+1):
            ...
            up[i,n,p]=0.1*um[i,n,p]*np.random.rand(1)
...
atm1 = np.zeros((nc,ns+1,nr))
print('Computing true POD coefficients...')
for p in range(nc):
    atm[p,:,:] = PODproj(um[:,:,p],PHI[:,:,p])
    atm1[p,:,:] = PODproj(um[:,:,p],PHIm[:,:,p])
...
def mean_squared_error_norm(y_true, y_pred):
    ...
    return K.mean(K.square((y_pred - y_true)/K.clip(K.abs(y_true),K.epsilon(),None)), axis=-1)

# create the LSTM architecture
model = Sequential()
#model.add(Dropout(0.2))
model.add(LSTM(40, input_shape=(lookback, n+1), return_sequences=True, activation='tanh'))
model.add(LSTM(40,  input_shape=(lookback, n+1), return_sequences=True, activation='tanh'))
...
#model.add(LSTM(40, input_shape=(lookback, n+1), return_sequences=True, activation='relu', kernel_initializer='uniform'))
model.add(LSTM(40, input_shape=(lookback, n+1), activation='tanh'))
model.add(Dense(n))
...
# run the model
history = model.fit(xtrain, ytrain, epochs=1200, batch_size=32, validation_split=0.2)
...
#%%
#PHItest2 = GrassInt2(PHI,pref,nu,nuTest)#%%

def plot_data(x,phiTrue,phiTest):
    fig, ax = plt.subplots(nrows=5,ncols=2,figsize=(12,8))
    ax = ax.flat
    nrs = phiTrue.shape[1]
    ...
    line_labels = ["Observed","Model"]#, "ML-Train", "ML-Test"]
    plt.figlegend( line_labels,  loc = 'lower center', borderaxespad=0.0, ncol=3, labelspacing=0.)
    plt.show()
    fig.savefig('grasman.pdf')

plot_data(x,PHI[:,:,-1],PHIm[:,:,-1]) 

#plot_data(x,PHItrue,PHItest2,PHItest) 
#plot_data(x,PHItest,PHItest2,PHI[:,:,1]) 

#%% LSTM [Fully Nonintrusive]
# testing
resTest = aTest - aGPtest
testing_set = np.hstack(( nuTest*np.ones((ns+1,1)), resTest  ))
...
for i in range(lookback):
    ...
    xtest[0,i,:]  = temp
    rLSTM[i, :] = testing_set[i,1:] 

# Prediction
for i in range(lookback,ns+1):
    #xtest_sc = scalerIn(xtest)
    ytest = model.predict(xtest)
    #ytest = scalerOut(ytest_sc)
    rLSTM[i, :] = ytest
