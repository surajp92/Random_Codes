...
indices = np.random.randint(0,xtrain.shape[1],8000)
xtrain1 = xtrain[:,indices]
ytrain1 = ytrain[:,indices]

#%%
model = Sequential()
model.add(Dense(10, input_shape=(8000,), init='uniform', activation='sigmoid', use_bias=True))
...
history = model.fit(xtrain1, ytrain1, nb_epoch=200, batch_size=200, validation_split=0.1) # run the model
...
#
## Hidden layers
#x = Dense(10, activation='sigmoid', kernel_initializer='glorot_normal', use_bias=True)(input_layer)# ,kernel_initializer='glorot_normal',
##x = Dense(10, activation='sigmoid', kernel_initializer='glorot_normal', use_bias=True)(x)
#op_val = Dense(n-1, activation='linear', kernel_initializer='glorot_normal', use_bias=True)(x)
#
...
#history_callback = custom_model.fit(xtrain, ytrain, epochs=1000, batch_size=100, verbose=1, 
