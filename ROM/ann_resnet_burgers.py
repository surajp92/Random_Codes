...
def create_training_data(training_set, dt):
    """ 
...
for k in range(l):
    p = int(k*m/l)
    q = int(p+1001)
    xt, yt = create_training_data(training_set[p:q,:], dt)
    if k == 0:
        xtrain = xt
        ytrain = yt
    else:
...
#%%
model = Sequential()
model.add(Dense(10, input_shape=(8,), kernel_initializer='uniform', activation='tanh', use_bias=True))
...
history = model.fit(xtrain1, ytrain1, nb_epoch=1000, batch_size=200, validation_split=0.1) # run the model
