...
fig.tight_layout()
plt.show()   
fig.savefig(f'plots_mg.png', dpi=300) 
...
for i in range(3):
    ...
    for j in range(3):
        ...
        data = np.load(f'solution_{int(re)}_{nx}_{nx}.npz')
...
fig.tight_layout()
plt.show()    
fig.savefig(f'contour_mg_w.png', dpi=300) 
...
fig.tight_layout()
plt.show()    
fig.savefig(f'contour_mg_s.png', dpi=300) 
