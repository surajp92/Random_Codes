...
for k in range(3):
    for j in range(2):
        iles = j + 1
        data = np.load(f'data_{iles}_{isp}_{nse}_{nx_list[k]}.npz')
        u = data['u']
        
        
        for ne in range(nse):    
            uf = np.fft.fft(u[ne,:,-1])
            eplot = spectra_calculation(nx_list[k],uf)
            if ne == 0:
                eplot1 = eplot
            else:
                eplot1 = eplot1 + eplot
        ...
        ax[k].loglog(kw,eplot1[1:], lw=2, label=labels[j])
...
# ax.set_xlim([1,kw[-1]+100])
    ax[j].legend()
    ax[j].set_title(f'$N = {nx_list[j]}$')
    ax[j].set_xlabel('$k$')
    ax[j].set_ylabel('$E(k)$')
        
plt.show() 
fig.tight_layout()
fig.savefig('grid_resolution.png', dpi=300)