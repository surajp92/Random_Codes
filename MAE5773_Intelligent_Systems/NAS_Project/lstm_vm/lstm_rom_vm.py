...
def plot_data(t,at,aGP):
    ...
    plt.show()
    fig.savefig('modes_gp.png')

plot_data(t,at[:,:,-1],aGP[:,:,-1])    
...
lookback = 8 #Number of lookbacks
...
model.add(LSTM(60, input_shape=(lookback, n+1), return_sequences=True, activation='tanh'))
#model.add(LSTM(40, input_shape=(lookback, n+1), return_sequences=True, activation='relu', kernel_initializer='uniform'))
model.add(LSTM(60, input_shape=(lookback, n+1), activation='tanh'))
model.add(Dense(n))
...
# run the model
history = model.fit(xtrain, ytrain, epochs=800, batch_size=32, validation_data= (xvalid,yvalid))
...
# Save the model
filename = './plotting/h1_model.hd5'
model.save(filename)
...
# testing
model = load_model('./plotting/h1_model.hd5', custom_objects={"coeff_determination":coeff_determination})
...
for i in range(lookback):
    xtest[0,i,0] = nuTest
    xtest[0,i,1:]  = testing_set[i,:].reshape(1,-1)
    aml[i,:] = testing_set[i,:] # modified GP + correction = True
...
for i in range(lookback,ns+1):
    ...
    xtest[0,lookback-1,1:] = aml[i,:] 

print('Reconstructing with true coefficients for test Re')
w_ml = PODrec(aml[:,:],PHIwtest[:,:])
...
#%%
modal_coeffs = np.hstack((aTest,aGPtest,aml))
...
#%%
def plot_data_modes(t,aTrue,aGPtest,aGPmlc1,filename):
...
plot_data_modes(t,aTest,aGPtest,aml,'ns2d_modes_'+str(int(ReTest))+'.pdf')
