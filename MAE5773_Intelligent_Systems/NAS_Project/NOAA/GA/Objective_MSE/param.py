...
class ContinuousParam(Param):
    ...
    def sample(self):
        ...
        value = np.random.randint(self.min_limit,self.max_limit+1,size=1)
...
class ContinuousParamGaussian(Param):
    def __init__(self, name, mean, variance, min_limit=None, max_limit=None, is_int=False):
        super(ContinuousParamGaussian, self).__init__(name)
        self.mean = mean
        self.variance = variance
        self.min_limit = min_limit
        self.max_limit = max_limit
...
if __name__ == "__main__":
    criterion_param = CategoricalParam("criterion", ["mse", "friedman_mse"], [0.8, 0.2])
    print(criterion_param.sample())
