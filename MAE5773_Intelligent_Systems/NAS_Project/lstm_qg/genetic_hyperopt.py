...
class GeneticHyperopt:
    def __init__(self, learner, X, y, fitness_func, maximize, num_folds=3,
                 pop_size=30, num_gen=10, elite_percent=0.1, competitiveness=0.4, mutation_prob=0.2):
    ...
    def _evaluation_function(self,y_true, y_pred):
...
#        print(type(learner_obj))
        ...
        for train_ind, val_ind in self.indices:
            learner_obj.fit(self.X[train_ind, :], self.y[train_ind], epochs=100, batch_size=64, verbose=0)
#            score += self.fitness_func(learner_obj.predict(self.X[val_ind, :]), self.y[val_ind])
            score += self._evaluation_function(learner_obj.predict(self.X[val_ind, :]), self.y[val_ind])
    ...
    def evolve(self):
        ...
        plotting_stats = np.zeros((self.num_gen,4))
        best_param_dict = {}
        ...
        for i in range(self.num_gen):
            ...
            plotting_stats[i,1] = np.min(fitness)
            plotting_stats[i,2] = np.max(fitness)
            plotting_stats[i,3] = np.mean(fitness)
